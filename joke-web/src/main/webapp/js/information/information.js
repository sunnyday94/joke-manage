...
function saveinformation() {
	...
	if ($("#addinformation").valid() && num == 0) {
		...
		$("#addinformation").attr("action",
				"addInformation.htm?CSRFToken=" + $("#CSRFToken").val());
...
function updatemodal(infoid) {
	$.ajax({
		...
		success : function(data) {
			...
			if (data.userDefined != null && data.userDefined != ""
					&& data.userDefined != undefined) {
				var userDefineds = data.userDefined.split(',');
				$.each(userDefineds, function(index, us) {
					$("#up_infoUD input:checkbox").each(
							function(index, infoud) {
								if (us == $(this).val()) {
...
function updateinformation() {
	...
	if ($("#updateInformation").valid()) {
		...
		$("#updateInformation").attr("action",
				"updateInformation.htm?CSRFToken=" + $("#CSRFToken").val());
...
function delinformation() {
	...
	if (checkedList.length > 0) {
		$.post('delInformation.htm?CSRFToken=' + $("#CSRFToken").val(), {
			infoIds : checkedList
		}, function() {
