...
$(function() {
	/* 查询栏目放在树形控件中 */
	$.get("selectAllInfoType.htm?CSRFToken=" + $("#CSRFToken").val(), function(
			data) {
		addInfoTypes(data, 1);
	});
	/* 获取会员等级 */
	$.get("getPointLevel.htm?CSRFToken=" + $("#CSRFToken").val(),
			function(data) {
				for (var i = 0; i < data.length; i++) {
					var op = $("<option></option>").val(data[i].pointLelvelId)
							.text(data[i].pointLevelName);
...
function addgradeInfo(infoTypes, grade) {
	...
	for (var i = 0; i < infoTypes.length; i++) {
		if (infoTypes[i].grade == grade) {
			...
			if (infoTypes[i].childs.length > 0) {
				str += addgradeInfo(infoTypes[i].childs, grade + 1,
						infoTypes[i].parentId);
			}
		}
	}
	return str;
...
function updateinformationType() {
	if ($("#updateInformationType").valid()) {
		if (!checkAddInfoTypeName($("#up_name").val(), $("#up_infoTypeId")
				.val())) {
			updateTips("栏目名称已存在!", $("#up_name_tips"));
			return;
		}
		var infoTypeId = $("#up_infoTypeId").val();
		var parentId = $("#up_parentId").val();
		if (parentId == infoTypeId) {
			updateTips("不能使用本身为上级栏目", $("#up_name_tips"));
			return;
		}
...
function delinformation() {
	location.href = "delInfoType.htm?CSRFToken=" + $("#CSRFToken").val()
			+ "&infoTypeId=" + $("#delinfoId").val();
}