...
function submitAreaAddForm(formId,btnObj) {
    ...
    if(formId=='saveProvinceForm'){
         ...
         * 验证省份名称是否已存在
         * */
        var provinceName = $("#addProvince").find("input[name='addressName']").val();
         $.ajax({
            url:"ajaxqueryprovincename.htm?CSRFToken="+$("#CSRFToken").val()+"&provinceName="+provinceName,
             type:"post",
             async:false,
             success:function(provinceNameCount){
                 if(provinceNameCount > 0){
                     showTipAlert("该省份名称已存在！");
                     checkName = false;
                 }
             }
        });
         ...
         * 验证城市名称是否已存在
         * */
        var cityName = $("#addCity").find("input[name='addressName']").val();
        var provinceId = $("#addCity").find("select[name='parentId']").val();

        $.ajax({
            url:"ajaxquerycityname.htm?CSRFToken="+$("#CSRFToken").val()+"&cityName="+cityName+"&provinceId="+provinceId,
            type:"post",
            async:false,
            success:function(cityNameCount){
                if(cityNameCount > 0){
                    showTipAlert("该城市名称已存在！");
                    checkName = false;
                }
            }
        });
         ...
         * 验证新增的区县名称是否存在
         * */
        var districtName = $("#addCounty").find("input[name='addressName']").val();
        var cityId = $("#addCounty").find("select[name='parentId']").val();
        $.ajax({
            url:"ajaxquerydistrictname.htm?CSRFToken="+$("#CSRFToken").val()+"&cityId="+cityId+"&districtName="+districtName,
            type:"post",
            async:false,
            success:function(districtNameCount){
                if(districtNameCount > 0){
                    showTipAlert("该区县名称已存在！");
                    checkName = false;
                }
            }
        });
