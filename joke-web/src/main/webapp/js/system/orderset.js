...
$(function() {
	...
	$('.form_value').click(function() {
		...
		if (!$('.form_btns').is(':visible')) {
			...
			if (formItem.attr("attr_flag") == 4) {
				$('.limit_btns').show();
				flag = '.limit_btns';
			}
			if (formItem.attr("attr_flag") == 5) {
				$('.is_push_btns2').show();
				flag = '.is_push_btns2';
			}
			if (formItem.attr("attr_flag") == 6) {
				$('.push_limit_btns').show();
				flag = '.push_limit_btns';
			}
			$(flag).css({
				'left' : formItem.parent().position().left + (formItem.next().find(".form-control").width() == null 
						? formItem.next().width() : formItem.next().find(".form-control").width()) + 65 + 'px',
	...
	$('.form_cancel').click(function() {
		...
		$('.receipt_btns').hide();
		$('.limit_btns').hide();
		$('.push_limit_btns').hide();
		$('.is_push_btns2').hide();
		formItem.parent().removeClass('form_open');
	});
	$('.btn-save').click(function() {
		var newValue = $('input[name="isBackOrder"]:checked').val();
		$.post("updatesetBackOrder.htm?CSRFToken=" + CSRFToken, {
	...
	$('.btn-time-save').click(function() {
		if (!isNaN($("#bsetOrderTime").val()) && $("#bsetOrderTime").val() != "") {
			$.post("updatesetOrderTime.htm?CSRFToken=" + CSRFToken, {
	...
	$('.btn-limit-save').click(function() {
		if (!isNaN($("#limitOrderTime").val()) && $("#limitOrderTime").val() != "") {
			$.post("updatesetBackOrder.htm?CSRFToken=" + CSRFToken, {
				limitOrderTime : $("#limitOrderTime").val(),
				setId : $("#setId").val()
			}, function(data) {
				if (data.result == 1) {
					$(formItem).text($("#limitOrderTime").val() + '天');
				}
			});
		}
		$('.limit_btns').hide();
	...
	$('.btn-pushLimit-save').click(function() {
		if (!isNaN($("#pushLimit").val()) && $("#pushLimit").val() != "") {
			$.post("updatesetBackOrder.htm?CSRFToken=" + CSRFToken, {
				pushLimit : $("#pushLimit").val(),
				setId : $("#setId").val()
			}, function(data) {
				if (data.result == 1) {
					$(formItem).text($("#pushLimit").val() + '小时');
				}
			});
		}
		$('.push_limit_btns').hide();
	...
	$('.btn-isPush-save').click(function() {
		var newValue = $('input[name="isPush"]:checked').val();
		$.post("updatesetBackOrder.htm?CSRFToken=" + CSRFToken, {
			...
			if (data.result == 1) {
				...
				$(formItem).html(oldValue == '0' ? '<span class="label label-success">是</span>' : '<span class="label label-default">否</span>');
			}
		});
		$('.is_push_btns2').hide();
		formItem.parent().removeClass('form_open');
