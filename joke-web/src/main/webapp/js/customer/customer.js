...
$(function() {
	...
	$("#customerUsername").blur(function() {
		if (modified != 1 && $(this).val() != "") {
			checkExist("customerUsername",
					$("#customerUsername").val(),
					"checkExistCustomerUsername.htm", "用户名");
		}
	});
	...
	$("#email").blur(function() {
		if (modified != 1 && $(this).val() != "") {
			checkExist("email", $("#email").val(),
					"checkbossemailexist.htm", "邮箱");
		} else if (modified == 1) {
			if ($("#email").val() != ""
					&& oldemail != $("#email").val()) {
				checkExist("email", $("#email").val(),
	...
	$.post("getPointLevel.htm?CSRFToken=" + $("#hi_token").val(), function(data) {
		for (var i = 0; i < data.length; i++) {
			if (data[i].isDefault == 1) {
				...
				$('#pointlevel').html(pointLevel.pointLevelName);
			}
		}
	});
	// 填充省份
...
function doSearchCustomer(customerId) {
	...
	$.post("queryCustomerById.htm?CSRFToken=" + $("#hi_token").val() + "&customerId=" + customerId, function(data) {
		...
		$("#pointlevel").text(data.pointLevelName);
		$('#email').hide();
		...
		if (data1 != '' && data1.length != 0) {
			...
			$.post("getAllDistrictByCid.htm?CSRFToken=" + $("#hi_token").val() + "&cityId=" + data.infoCity, function(data1) {
				...
				if (data1 != '' && data1.length != 0) {
					...
					$.post("getAllStreetByDid.htm?CSRFToken="
							+ $("#hi_token").val() + "&dId=" + data.infoCounty,
							function(data1) {
								if (data1 != '' && data1.length != 0) {
									var options = "<option value='' >" + "选择街道" + "</option>";
									for (var i = 0; i < data1.length; i++) {
...
/**
 * propName 属性名 value 属性值 url 查询路径 tipStr 提示内容
 */
function checkExist(propName, value, url, tipStr) {

	if ($("#" + propName).hasClass("error")) {
		return false;
	}
