            ...
            if(data != null){
             for(var i=0;i<data.length;i++) {
                if(data[i].backLogStatus == 7){
                        s += '<td>退款成功</td><td>' + data[i].backLogPerson + '</td>'
                    }
                 if(data[i].backLogStatus == 6){
                     s += '<td>收货失败</td><td>' + data[i].backLogPerson + '</td>'
                 }
                 if(data[i].backLogStatus == 8){
                     s += '<td>同意退款</td><td>' + data[i].backLogPerson + '</td>'
                 }
                 if(data[i].backLogStatus == 9){
                     s += '<td>拒绝退款</td><td>' + data[i].backLogPerson + '</td>'
                 }
                 if(data[i].backLogStatus == 5){
                     s += '<td>确认收货</td><td>' + data[i].backLogPerson + '</td>'
                 }
                 if(data[i].backLogStatus == 4){
                     s += '<td>待填写快递单号</td><td>顾客</td>'
                 }if(data[i].backLogStatus == 3){
                     s += '<td>申请审核不通过</td><td>' + data[i].backLogPerson + '</td>'
                 }if(data[i].backLogStatus == 2){
                     s += '<td>申请审核通过</td><td>' + data[i].backLogPerson + '</td>'
...
// 退货操作
function backdetail(backId,orderId,businessId) {
    window.frames["mainFrame"].document.body.innerHTML = "";
    $(".remark_content").text("");
    $("#mainFrame").attr("src",'newbackdetail.htm?CSRFToken='+$("#CSRFToken").val()+'&backId='+backId+'&orderId='+orderId+'&businessId='+businessId);
    $('#dialog-Back-backCheck').modal('show');
    $('#backOrderId').val(backId);
    if(businessId == 1){
...
// 退款操作
function backprice(backId,orderId,businessId) {
    window.frames["mainFrames"].document.body.innerHTML = "";
    $(".remark_contents").text("");
    $("#mainFrames").attr("src",'newbackprice.htm?CSRFToken='+$("#CSRFToken").val()+'&backId='+backId+'&orderId='+orderId+'&businessId='+businessId);
    $('#dialog-Back-price').modal('show');
    if(businessId == 1){
        $('#mainFrames').css('minHeight','250px');
    }else{
        $('#mainFrames').css('minHeight','300px');
    }
}
...
function saveorder() {
    if($(".flag_saved").val()=="1"){
    	...
    	var customerRemarkContent = $(".customerRemark").val();
		if($.trim(customerRemarkContent).length == 0){
			$(".remark_content").text("请填写留言！");
            return;
		}
        ...
        if(backLogStatus.val() == "5") {
            $("input[name='backLogStatus']").val("7");
            if (checkBackPrice == "") {
        ...
        }else if(backLogStatus.val() == "6"){
            $("input[name='backLogStatus']").val("6");
        }
        $(".flag_saved").val("1");
...
function showPrice(obj){
    if(obj==1){
        ...
        $.ajax({
            type: 'post',
            url: 'ispointenough.htm?orderId='+ orderId,
            async: false,
            success: function(data){
               if (data ==1)
               {
...
function checkBackMoney(){
    ...
    var money = $(".checkBackPrice").val();
    if(money !=""){
        $.ajax({
            type: 'post',
            url: 'checkbackprice.htm?backOrderId='+ backorderid,
            async: false,
            success: function(data){
                if(money > data.backPrice){
                    $(".checkBackPrice").addClass("error");
                    $("#backMoneytip").show();
                }else{
                    $(".checkBackPrice").removeClass("error");
                    $("#backMoneytip").hide();
                    result = true;
                }
            }
