...
function addstatusstr(str, time, num, cla) {
	if (time != null) {
		return '<li class="' + cla + '">' + '<p class="name">' + str + '</p>'
				+ '<p class="bar"><i>' + num + '</i></p>' + '<p class="time">'
				+ timeObject(time) + '</p>' + '</li>';
	} else {
		return '<li class="' + cla + '">' + '<p class="name">' + str + '</p>'
		...
		success : function(data) {
			...
			if (data != "" && data != null && data != undefined) {
				...
				if (order != null) {
					...
					//$("#detailVoucherNo").html("代金券码：" + notNull(order.voucherNo));
					//$("#detailVoucherName").html("代金券名称：" + notNull(order.voucherName));
					...
					if (order.invoiceType == "0") {
						var invoicestr = '<div class="col-sm-12">' + '<p>发票类型：不需要发票</p>' + '</div>' + '<div class="col-sm-12">';
						...
						var invoicestr = '<div class="col-sm-12">'
								...
								+ '<div class="col-sm-12">'
								+ '<p>发票抬头：'
								+ notNull(order.invoiceTitle) + '</p>'
								+ '</div>' + '<div class="col-sm-12">'
								+ '<p>发票内容：'
								+ notNull(order.invoiceContent)
								+ '</p>' + '</div>'
								+ '<div class="col-sm-12">';
						if (order.orderLinePay == '1') {
							invoicestr += '<p>支付类型：在线支付</p>';
					...
					/* 订单备注(后台添加) */
					if (data.orderRemark != null && data.orderRemark.length >= 1) {
						var html = "";
						$.each(data.orderRemark, function(index, item) {
							html += '<tr>';
							html += '<td>' + item.orderRemark + '</td>';
							html += '<td>' + item.addUserName + '</td>';
							html += '<td>' + timeObject(item.addRemarkTime) + '</td>';
							html += '</tr>';
						});
						$("#Js_orderRemarks").html(html);
						$(".Js_orderRemark").attr("style", "display: ;"); // 显示订单备注 
					} else {
						$(".Js_orderRemark").attr("style", "display: none;"); // 不显示订单备注
					}
						...
						if (item.goodsProductReleSpecVoList != null) {
							...
							for (var s = 0; s < item.goodsProductReleSpecVoList.length; s++) {
								str += item.goodsProductReleSpecVoList[s].spec.specName + ':'
										+ item.goodsProductReleSpecVoList[s].specValueRemark
							}
							str += '</td>';
						}
					...
					if (orderLogs != null && orderLogs != undefined) {
						...
						$.each(orderLogs, function(idx, item) {
							...
							if (item.orderLogStatus == '0') {
								...
								strtk += '<tr><td>退款</td>';
								strtk += '<td>' + timeObject(item.orderLogTime)
										+ '</td>';
								strtk += '<td>' + item.orderLogReason
										+ '</td></tr>';
							} else if (item.orderLogStatus == '6') {
								strLog += '<td>修改状态</td>';
							}
							strLog += '<td>' + item.orderLogPerson + '</td>';
							strLog += '<td>' + timeObject(item.orderLogTime)
									+ '</td>';
							strLog += '<td>' + item.orderLogReason
									+ '</td></tr>';
						});
						if (strtk != "") {
							$("#ordertk").html(strtk);
							...
							$("#ordertk")
									.html(
											'<tr><td colspan="3"><p class="text-center">暂无可用数据！</p></td></tr>');
						}
						if (strLog != "") {
							...
							$("#orderLog")
									.html(
											'<tr><td colspan="4"><p class="text-center">暂无可用数据！</p></td></tr>');
						}
					}
...
function getExpressInfo(oId) {
	$.ajax({
		type : "POST",
		url : "queryExpressInfo.htm",
		data : "orderId=" + oId,
		success : function(data) {
			$(".J_orderCode").html("订单编号：" + (data.orderCode == undefined ? "" : data.orderCode));
			$(".J_expressName").html("物流公司名称：" + (data.expressName == undefined ? "" : data.expressName));
			$(".J_expressCode").html("运单号：" + (data.num == undefined ? "" : data.num));
			var expressInfo = "";
			var express = data.express;
			if (express != null && express != "") {
				for (var i = 0; i < express.length; i++) {
					expressInfo += '<tr id="expressInfo"><td>' + express[i].context + '</td>';
					expressInfo += '<td>' + express[i].time + '</td></tr>';
				}
				$("#expressInfo").html(expressInfo);
			} else {
				$("#expressInfo").html('<tr><td colspan="4"><p class="text-center">暂无可用数据！</p></td></tr>');
			}
...
function pagegoods(pageNo, rows) {
	...
	var strpage = "";
	if (pageNo > 1) {
		strpage += '<a href="javascript:;" onclick="pagegoods(' + (pageNo - 1)
				+ ',' + rows + ')"> 上一页 </a>';
	} else {
	...
	for (var i = 1; i <= rows; i++) {
		if (i == pageNo) {
			...
			strpage += '<a href="javascript:;" onclick="pagegoods(' + i + ','
					+ rows + ')">' + i + ' </a>';
		}
	}
	if (pageNo < rows) {
...
function pagelogs(pageNo, rows) {
	...
	var strpage = "";
	if (pageNo > 1) {
		strpage += '<a href="javascript:;" onclick="pagelogs(' + (pageNo - 1)
				+ ',' + rows + ')"> 上一页 </a>';
	} else {
		strpage += '<span class="disabled"> 上一页 </span>';
	}
	for (var i = 1; i <= rows; i++) {
		if (i == pageNo) {
			...
			strpage += '<a href="javascript:;" onclick="pagelogs(' + i + ','
					+ rows + ')">' + i + ' </a>';
		}
	}
	if (pageNo < rows) {
...
function timeObject(obj) {
	...
	var time = new Date(parseInt(date.replace("/Date(", "").replace(")/", ""),
			10));
	var result = time.getFullYear()
			+ "-"
			+ (time.getMonth() + 1 < 10 ? "0" + (time.getMonth() + 1) : time
					.getMonth() + 1)
			+ "-"
...
function fnModifyOrder(oId, sta, orderLinePay) {
	...
	$("#updateOrderst").attr(
			"action",
			"modifyOrderStatus.htm?orderId=" + oId + '&CSRFToken='
					+ $("#CSRFToken").val());
...
function upmoney() {
	...
	if (price > 0 && checkReason() && expressprice > 0) {
...
// 验证身份
function upmoneypwd() {
	$("#upmoneypwd").attr("disabled", "disabled");
	...
	if (pwd != null && pwd.trim().length != 0) {
		...
		$.ajax({
			...
			success : function(data) {
				if (data > 0) {
					$("#upmoneyform").attr("action", "modifyorderprice.htm")
							.submit();
				} else if (data == 0) {
					$(".pwdtip").text("密码错误!");
 ...
 * @param code
 */
function fnModifyOrderCancel(oId, sta) {
	$("#orReason").val("");
	$("#oStatus").val(sta);
	$("#orId").val(oId);
	// 确认中断框
...
function verifylogin() {
	...
	var pwd = $("#pwd").val();
	if (pwd != null && pwd.trim().length != 0) {
		var url = 'checkUserKey.htm?userKey=' + pwd + '&CSRFToken='
				+ $("#CSRFToken").val();
		$.ajax({
			type : 'post',
			url : url,
			...
			success : function(data) {
				if (data > 0) {
					if (digit == 0) {
						$("#updateOrder").attr("action", "suspendorder.htm")
								.submit();
						digit++;
					}
...
function submitOrder() {
	...
	$("#orderStatusstr").val($("#orderStatus").val());
	$("#customerUsernamestr").val($("#customerUsername").val());
	$("#couponNamestr").val($("#couponName").val());
	$("#flagstr").val($("#flag option:selected").val())
	$("#endTimestr").val($("#endTime").val());
 ...
 * 展示一键发货dialog
 */
function showOneKeyDelivery(orderId, orderMType,businessId) {
	var token = $("#CSRFToken").val();
	$.ajax({
		type : 'get',
			...
			$.ajax({
				...
				success : function(data) {
					if (data == true) {
						...
						/*if ($("#orderMType").val() == '2') {
							$.post("sendgoodsnotify.htm?CSRFToken="
									+ $("#CSRFToken").val(), $(
									"#oneKeyDeliveryForm").serialize(),
									function(data) {
										location.reload();
									});
						} else {
...
// 打开添加订单备注输入框
function toAddOrderRemark(orderId) {
	var token = $("#CSRFToken").val();
	$("#remark").val("");
	$("#toAddOrderRemarkForm").find("input[name='orderId']").val(orderId);
	$.ajax({
		type : 'get',
		url : "toAddOrderRemark.htm?orderId=" + orderId + "&CSRFToken=" + token,
		async : false,
		success : function (data) {
			$("#toAddOrderRemarkForm").find("input[name='orderCode']").val(data.orderCode);
			$("#Js_orderCode").text(data.orderCode);
			$("#toAddOrderRemark").modal('show');
		}
	});
...
function addOrderRemark() {
	if ($("#remark").val() == '') {
		$("#reasontip").html("备注内容不能为空");
		return false;
	}

	$.ajax({
		type : 'post',
		url : 'addOrderRemark.htm?CSRFToken=' + $("#CSRFToken").val(),
		data : $("#toAddOrderRemarkForm").serialize(),
		async : false,
		success : function(data) {
			if (data == 1) {
				showTipAlert("订单添加备注成功！");
				location.reload();
			}
		},
		error : function() {
