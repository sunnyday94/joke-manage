				...
				if (event.keyCode == 13 || event.keyCode == 9 || event.type == "blur") {
					...
					if (value != "") {
						if (value.length > opts.tabMaxLen) {
							alert("请输入1到" + opts.tabMaxLen + "个字符长度的标签");
							return;
						}
                        if(opts.regularEx!=''){
                            var regu = new RegExp(opts.regularEx);
                            if(!regu.test(value)){
                                alert('请输入符合条件的内容');
                                return;
                            }
						...
						if (!_match) {
							...
							alert("內容不能包含非法字符「{0}」！".replace("{0}", _match.join(" ")));
						}
...
**/
$.fn.extend({
	tabControlEmail: function(options, tags) {
		var defOpt = {
			initTabCount: 1,
			maxTabCount: 10,
			tabMaxLen: 100,
		...
		var checkReg = /[^A-Za-z0-9_\u4E00-\u9FA5\.@]+/gi;//匹配非法字符
		var initTab = function(obj, index) {//初始化标签输入
			var textHtml = "<input class='' name='tabinput' style='width:" + opts.tabW + "px;height:" + opts.tabH + "px;' type='text'/>";
			obj.append(textHtml);
			if (_tags[index]) {
				var __inputobj = $("input[type='text'][name='tabinput']", obj).eq(index);
				__inputobj.val(_tags[index].substr(0, opts.tabMaxLen)).css("display", "none");
