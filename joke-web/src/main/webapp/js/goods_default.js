	...
	if (e.hasClass("open_sec")) {
		...
		for (var i = 1; i <= n; i++) {
			...
			if (tr.attr("class").substring(12, 13) > num) {
				tr.hide();
				tr.find(".close_sec").removeClass("close_sec").addClass(
						"open_sec");
			}
...
function addTab(title, url) {
	...
	var tabs = $("#tabs").tabs();
	var label = title || "Tab " + tabCounter, id = "tabs-" + tabCounter, li = $(tabTemplate.replace(/#\{href\}/g, "#" + id).replace(/#\{label\}/g, label));
...
/* 验证是否已经存在的通用方法 */
/**
 * url 验证控制器 objName 字段名称 obj input控件 objTips 提示框class名称 flag 是新添加 还是修改 oldObj
 * 如果是修改 传过来旧值 如果输入的和旧值相同 就不验证
 */
 ...
 * @param sel_id
 */
function checkProductParam(objTips, flag, up_remark_id, sel_id) {
	var type = $("#addOrUpdate").val();
	var params = $(".up_sel_spec");
	if (flag == 0) {// 添加
		var url = "checkParam.htm?goodsId=" + $("#goodsId").val()
				+ "&paramLength=" + $("#paramLength").val();
		for (var i = 0; i < params.length; i++) {
			url += "&paramIds=" + $(params[i]).val();
		}
		$.ajax({
