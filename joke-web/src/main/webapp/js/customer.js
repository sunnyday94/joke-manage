...
$(function() {
	...
	$("#dialog-tip1").hide();
	$("#dialog-err-tip").hide();
	$("#dialog-report-gen").hide();
	$("#dialog-report-exp").hide();
	$("#city").hide();
	$("#district").hide();
	$("#street").hide();
	$("#city_a").hide();
...
//					}
//					alert(modified);
					bValid =  checkLength(username, "用户名", 4, 20, usernametip) && bValid;
					bValid =  bValid && checkRegexp(username, /^([0-9a-z_])+$/i, "用户名可包含a-z, 0-9,下划线", usernametip) ;
					if(modified != 1){
...
//			$(".user_dl").find(".p_div ").remove();
	...
	$("#del-id").button().click(function() {
		if(checkSelected('storeId',1)){
			$("#dialog-del-id").dialog("open");
		}else{
			$("#dialog-tip").dialog({
				resizable : false,
				height : 150,
	...
	$("#dialog-del-id").dialog({
		...
		buttons : {
			"确定" : function() {
				$.post("delectstore.htm",{storeIds:checkedList},function(result){  
				      if (result > 0){
				    	  location.href="auditlist.htm?checkStatus=1";
				      } else { 
	...
	$("#del-id1").button().click(function() {
		if(checkSelected('storeId',0)){
			$("#dialog-del-id1").dialog("open");
		}else{
			$("#dialog-tip").dialog({
				resizable : false,
				height : 150,
	...
	$("#del-ids1").button().click(function() {
		if(checkSelected('storeId',0)){
			$("#dialog-del-id1").dialog("open");
		}else{
			$("#dialog-tip").dialog({
	...
	$("#dialog-del-id1").dialog({
		...
		buttons : {
			"确定" : function() {
				$.post("delectstore.htm",{storeIds:checkedList},function(result){  
				      if (result > 0){
				    	  location.href="auditlist.htm?checkStatus=0";
				      } else { 
	...
	//设置会员类型
	$("#set-customerType").button().click(function() {
		if(checkSelected('customerid',0)){
			$("#dialog-set").dialog("open");
			/*$("#customerId").val($("input[name='customerid']:checked").val());
			$('#setForm').attr('action', 'setcustomer.htm?CSRFToken='+$("#hi_token").val());
			doSetCustomer($("input[name='customerid']:checked").val());*/
		}else{
			$("#dialog-tip").dialog({
				resizable : false,
				height : 150,
	...
	$("#dialog-set").dialog({
		...
		buttons : {
			"确定" : function() {
				//$("#setForm").submit();
				setMultiWin('setcustomer.htm?CSRFToken='+$("#hi_token").val(),"initCustomer.htm");
				$(this).dialog("close");
			},
			"取消" : function() {
				$(this).dialog("close");
...
/*ajax 通过ID查询并塞值到修改页面*/
function doSetCustomer(customerId){	
	modified=1;
	$.post("queryCustomerById.htm?CSRFToken="+$("#hi_token").val()+"&customerId="+customerId,function(data){
		 	 if(data.isSiteManager==0){
		 		$('#isSiteManager1').prop('checked', 'checked');
		 	 }else{
		 		$('#isSiteManager2').prop('checked', 'checked');
		 	 }
			 $("#dialog-set").dialog("open").dialog("option", { title : "设置会员" });
	});
}
 ...
 */
function setMultiWin(url,initUrl) {
	var siteManager = $('input[name="isSiteManager"]:checked').val();
      $.post(url,{parameterIds:checkedList,isSiteManager:siteManager},function(result){  
	      if (result > 0){
...
$(function() {
	...
	$("#create-user").button().click(function() {
		modified = 0;
		$("#customerImg").prop("name","customerImg");
...
//			pointNeed1.val("");
	...
	$("button[id='btn-toIndex']").button().click(function() {
		if(checkSelected('shareId',0)){
			var url = 'checkindexsharecount.htm?CSRFToken='+$("#hi_token").val()+'&count='+checkedList.length,iFlag =false;
			$.ajax({
		         type: 'post',
		         url:url,
		         async:false,   
...
//						$("#mod_pwd_from").submit();
	...
	$("#dialog-audit-confirm").dialog({
		...
		buttons : {
			"确定" : function() {
				...
				//验证输入框不能为空
				var vn = 0;
				$.each($("input[name='deduction']"),function(){
					if($(this).val() == '' || !re1.test($(this).val())){
						$(this).addClass("ui-state-error");
						vn ++;
						updateTips("请输入数字", $("#paywayTips"));
						return;
					}else {
						updateTips("", $("#paywayTips"));
				...
				$.each($("input[name='brokerage']"),function(){
					if($(this).val() == '' || !re1.test($(this).val())){
						$(this).addClass("ui-state-error");
						vn ++;
						updateTips("请输入数字", $("#paywayTips"));
				...
				if($("#aTime").val() == ""){
					updateTips("请添加结算周期", $("#empTimeTips"));
					return;
				}else{
					updateTips("", $("#empTimeTips"));
				}
	...
	$("#dialog-audit-confirm1").dialog({
		...
		buttons : {
			"确定" : function() {
				...
				$("#audit_from").prop("action","updateClassifyPay.htm?CSRFToken="+$("#hi_token").val());
				$("#audit_from").submit();
				$(this).dialog("close");
	...
	$("#audit-store").button().click(function() {
		...
		if(checkSelected('storeId',0)){
			...
			$("#storeId").val($("input[name=storeId]:checked").val());
			 $.ajax({
		         type: 'post',
			 ...
			 $.ajax({
		         type: 'post',
		         url:'findBusinessCircles.htm?CSRFToken='+$("#hi_token").val(),
		         async:false,
		         success: function(data) {
		        	 var h = "";
		        	 ...
		        	 if(data!=null){
		    	       	 for(var i = 0 ;i< data.length;i++){
		    	       		 h+='<option value="'+data[i].businessCircleId+'">'+data[i].businessCircleName+'</option>';
		    	       	 }
		             }
		        	 $("#businessCircle").html(h);
		         }
			 });
	...
	$("#search-audit1").button({
		...
		if(checkInputContent()){
			$('#search_from').attr('action', 'hadauditlist.htm?checkStatus=1&showFlag=0&attr='+$(".search_sel span").text()+"&attr="+$(".search_text").val());
			$("#pageSize").val(5);
			$("#pageNo").val(1);
			$('#search_from').submit();
		}else{
