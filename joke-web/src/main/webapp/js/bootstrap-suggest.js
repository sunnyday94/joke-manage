	...
	var methods = {
		init: function(opts){
			...
			return self.each(function(){
				var $input = $(this),
					$dropdownMenu = $input.parent(".input-group").find("div.dropdown-menu");
				//验证输入框对象是否符合条件
			...
			*/
			...
			function refreshDropMenu($input, data, opts) {
				var $dropdownMenu = $input.parent().find("div.dropdown-menu"),
				len, i, j, index = 0,
		...
		show: function(){
			...
			if (data && data.options) {
				this.parent().find("div.dropdown-menu").show();
			}
			return this;
		},
		hide: function(){
			var data = this.data('bsSuggest');
			if (data && data.options) {
				this.parent().find("div.dropdown-menu").css("display","");
			}
			return this;
		},
		disable: function (e) {
