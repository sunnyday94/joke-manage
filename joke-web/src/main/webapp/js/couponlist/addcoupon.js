...
function subFormOne() {
	...
	if ($("#summernote").code().replace(reg1, "").replace(reg2, "").replace(reg, "") != "<br>") {
		$("#couponDescription").val($("#summernote").code().replace(reg1, "").replace(reg2, "").replace(reg, "").replace(reg3, ""));
	}
	if ($("#summernote2").code().replace(reg1, "").replace(reg2, "").replace(reg, "") != "<br>") {
		$("#couponRuleDescription").val($("#summernote2").code().replace(reg1, "").replace(reg2, "").replace(reg, "").replace(reg3, ""));
	}
...
//		}
		...
		var cate = document.getElementsByName("cateIdp");
		if (cate.length == 0) {
			$("#ps1").html('请选择类目');
			$("#ps1").attr("style", "color:red");
			f = false;
		} else {
			f = true;
			$("#ps1").html('');
		...
		var brand = document.getElementsByName("brandIdP");
		if (brand.length == 0) {
			$("#ps2").html('请选择品牌');
			$("#ps2").attr("style", "color:red");
			f = false;
		} else {
			f = true;
			$("#ps2").html('');
...
function trueCate() {
	...
	for (var i = 0; i < nodes.length; i++) {
		if (!nodes[i].isParent) {
			...
			htm += '<td width="277">' + nodes[i].name + '</td>';
			htm += '<td width="108"><button onclick="removeTr(this);">移除</button></td>';
			htm += "</tr>";
		}
	}
	$("#list tbody").html(htm);
...
function chooseBrand() {
	...
	if (brandstr != null) {
		...
		for (var i = 0; i < $arr.length; i++) {
			...
			htm += '<td width="277">' + $("#brand option[value='" + $arr[i] + "']").text() + '</td>';
			htm += '<td width="108"><button onclick="removeTr(this);">移除</button></td>';
			htm += "</tr>";
		}
	}
