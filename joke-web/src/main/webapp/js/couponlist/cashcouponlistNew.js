...
function getCouponAddress(couponId) {
	...
	var couponConfirm = '<div class="modal fade" id="coupon-confirm" tabindex="-1" role="dialog">' + '   <div class="modal-dialog modal-lg">'
			+ '        <div class="modal-content">' + '            <div class="modal-header">'
			+ '               <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>'
			+ '               <h4 class="modal-title">领取链接</h4>' + '            </div>' + '            <div class="modal-body">' + '                <span id="coupon-tip"></span>'
			+ '            </div>' + '<input type="text" id="content"/>' + '           <div class="modal-footer">'
			+ '             <button type="button" class="btn btn-primary"  onclick="copymethord()">复制</button>' + '   </div>' + '        </div>' + '    </div>' + '</div>';
...
function getScanCouponNums(couponId) {
	$.ajax({
		url : 'newselectCashcouponnolistajax.htm?couponId=' + couponId,
		async : false,
		success : function(data) {
			if (data != "") {
				if (data.pageBean != "") {
					...
					$.each(data.pageBean, function(idx, item) {
						...
						if (item.codeStatus == '0') {
							str += '</td><td><span class="label label-default">可领取</span>';
						}else if (item.codeStatus == '1') {
							str += '</td><td><span class="label label-default">未使用</span>'; 
						}else if (item.codeStatus == '2') {
							str += '</td><td><span class="label label-success">使用成功</span>';
						} else if (item.codeStatus == '4') {
							str += '</td><td><span class="label label-danger">使用失败</span>';
						}
				...
				$("#couponW").html("");
				$("#couponWPage").html("");
				var num = pageRows(listw, 10);
				$.each(listw, function(idx, item) {
					if (idx < 10) {
					...
					strc += '<td>' + item.codeNo;
					strc += '</td><td><span class="label label-default">可领取</span>';
					if (item.codeGetTime == null) {
						strc += '</td><td>';
					} else {
				...
				if (start > 2) {
					strpagec += '<a href="javascript:;" onclick="pagewcouponsno(1,' + parseInt(num) + ')">1</a>';
				...
				for (var i = start; i <= end; i++) {
					if (1 == i) {
						...
						strpagec += '<a href="javascript:;" onclick="pagewcouponsno(' + i + ',' + parseInt(num) + ')">' + i + ' </a>';
					}
				}
				if (num > end + 1) {
					strpagec += '<span class="current">...</span>';
					strpagec += '<a href="javascript:;" onclick="pagewcouponsno(' + parseInt(num) + ',' + parseInt(num) + ')">' + parseInt(num) + '</a>';
				}
				if (num > 1) {
					strpagec += '<a href="javascript:;" onclick="pagewcouponsno(2,' + num + ')">下一页 </a>';
				} else {
					strpagec += '<span class="disabled"> 下一页 </span>';
				}
				$("#couponWPage").html(strpagec);
				...
				$.each(list, function(idx, item) {
					if (idx < 10) {
						strc += '<tr id="couponsunusenum' + idx + '" class="">';
					} else {
						strc += '<tr id="couponsunusenum' + idx + '" class="couponUnGot">';
					}
					strc += '<td>' + item.codeNo;
					strc += '</td><td><span class="label label-default">未使用</span>';
					if (item.codeGetTime == null) {
						strc += '</td><td>';
					} else {
						strc += '</td><td>' + timeObject(item.codeGetTime);
				...
				$.each(listg, function(idx, item) {
					...
					strg += '<td>' + item.codeNo;
					strg += '</td><td><span class="label label-success">使用成功</span>';
					if (item.codeGetTime == null) {
						strg += '</td><td>';
					} else {
				...
				var listy = new Array();
				for (var i = 0; i < data.pageBean.length; i++) {
					if (data.pageBean[i].codeStatus == '4') {
				...
				$.each(listy, function(idx, item) {
					...
					stry += '<td>' + item.codeNo;
					stry += '</td><td><span class="label label-danger">使用失败</span>';
					if (item.codeGetTime == null) {
						stry += '</td><td>';
					} else {
...
function timeObject(obj) {
	...
	var result = time.getFullYear() + "-" + (time.getMonth() + 1 < 10 ? "0" + (time.getMonth() + 1) : time.getMonth() + 1) + "-"
...
/*未领取券码分页*/
function pagewcouponsno(pageNo,rows){
	var start = parseInt(getStart(pageNo, rows));
	var end = parseInt(getEnd(pageNo, rows));
	$("#couponWPage").html("");
	var strpage = "";
	if (pageNo > 1) {
		strpage += '<a href="javascript:;" onclick="pagewcouponsno(' + (pageNo - 1) + ',' + rows + ')"> 上一页 </a>';
	} else {
		strpage += '<span class="disabled"> 上一页 </span>';
	}
	if (start > 2) {
		strpage += '<a href="javascript:;" onclick="pagewcouponsno(1,' + parseInt(rows) + ')">1</a>';
		strpage += '<span class="current">...</span>';
	}
	for (var i = start; i <= end; i++) {
		if (pageNo == i) {
			...
			strpage += '<a href="javascript:;" onclick="pagewcouponsno(' + i + ',' + parseInt(rows) + ')">' + i + ' </a>';
		}
	}

	if (rows > end + 1) {
		strpage += '<span class="current">...</span>';
		strpage += '<a href="javascript:;" onclick="pagewcouponsno(' + parseInt(rows) + ',' + parseInt(rows) + ')">' + parseInt(rows) + '</a>';
	}

	if (pageNo < rows) {
		strpage += '<a href="javascript:;" onclick="pagewcouponsno(' + (pageNo + 1) + ',' + rows + ')"> 下一页 </a>';
	} else {
		strpage += '<span class="disabled"> 下一页 </span>';
	}

	$("#couponWPage").html(strpage);
	$("#couponW tr").attr("class", "couponUnGot");
...
function pageungotcouponsno(pageNo, rows) {
	...
	$("#couponsunusenum" + (pageNo * 10 - 1)).attr("class", "");
	$("#couponsunusenum" + (pageNo * 10 - 2)).attr("class", "");
	$("#couponsunusenum" + (pageNo * 10 - 3)).attr("class", "");
	$("#couponsunusenum" + (pageNo * 10 - 4)).attr("class", "");
	$("#couponsunusenum" + (pageNo * 10 - 5)).attr("class", "");
	$("#couponsunusenum" + (pageNo * 10 - 6)).attr("class", "");
	$("#couponsunusenum" + (pageNo * 10 - 7)).attr("class", "");
	$("#couponsunusenum" + (pageNo * 10 - 8)).attr("class", "");
	$("#couponsunusenum" + (pageNo * 10 - 9)).attr("class", "");
	$("#couponsunusenum" + (pageNo * 10 - 10)).attr("class", "");
}
 ...
 * 获取显示的开始页码数与
 * 
 * @see {@link com.ningpai.util.PageBean#getEnd()} 相对存在
 * 
 ...
 * @returns
 */
function sendCouponAll(couponId) {
	sendCouponAllUrl = 'sendVoucherAll.htm?couponId=' + couponId;
	showSendCouponAllConfirmAlert(sendCouponAllUrl,'确认要给所有用户发送代金券吗？');
 ...
 * @returns
 */
function doAjaxSendCouponAll(sendCouponAllUrl) {
	$.ajax({
		url : sendCouponAllUrl,
		async : false,
		success : function(data) {
			tip = "已经发送过的用户不在重复发送，总共发出" + data.count + "张代金券！";
			showTipAlert(tip);
		}
	});
}