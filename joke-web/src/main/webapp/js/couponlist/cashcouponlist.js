...
function deleteSingleCashcoupon(codeId) {
	showDeleteOneConfirmAlert("delcashcoupon.htm?codeId=" + codeId + '&CSRFToken=' + $("#csrf_token").val());
}
...
/* 优惠券详细信息列表 (旧方法，已放弃使用)*/
function getScanCoupons(codeNo) {
	$.ajax({
		url : 'searchCashcouponByIdAjax.htm?codeNo=' + codeNo,
		async : false,
		success : function(data) {
			if (data != "") {
				var cashcoupon = data.cashcoupon;
				var list = "";
				if (cashcoupon != "") {
					$("#cashcouponCode").text(cashcoupon.codeNo);
					$("#cashcouponStartTime").text(timeObject(cashcoupon.startTime));
					$("#cashcouponEndTime").text(timeObject(cashcoupon.endTime));
					$("#cashcouponCreateTime").text(timeObject(cashcoupon.createTime));
					$("#cashcouponModifyTime").text(timeObject(cashcoupon.modifyTime));
					$("#voucherPrice").text(parseFloat(cashcoupon.voucherPrice).toFixed(2));
					if(cashcoupon.codeStatus!=null){
						if (cashcoupon.codeStatus == '0') {
							$("#isUsed").text("可领取");
						}else if(cashcoupon.codeStatus=='1'){
							$("#isUsed").text("未使用");
						}else if (cashcoupon.codeStatus == '2') {
							$("#isUsed").text("使用成功");
						} else if (cashcoupon.codeStatus == '4'){
							$("#isUsed").text("使用失败");
						}
					}
					if(cashcoupon.codeNo!='guodanianzuohaofan'){
						if(cashcoupon.orderCode!=null){
							$("#orderCode").text(cashcoupon.orderCode);
						}
					}
				}
			}
 ...
 * @returns
 */
function sendCouponAll(couponId, couponRulesType) {
	sendCouponAllUrl = 'sendCouponAll.htm?couponId=' + couponId + '&couponRulesType=' + couponRulesType;
	showSendCouponAllConfirmAlert(sendCouponAllUrl);
...
function doAjaxSendCouponAll(sendCouponAllUrl) {
	$.ajax({
		...
		async : false,
		success : function(data) {
			tip = "已经发送过的用户不在重复发送，总共发出" + data.count + "张优惠券！";
			showTipAlert(tip);
		}
	});
}