		...
		success : function(data) {
			if (data != "") {
				...
				if (coupon != "") {
					...
					$("#couponDescription").text(notNull(coupon.couponDescription));
				...
				/*
				 * for(var i=2;i<=num;i++){ strpage +='<a href="javascript:;"
				 * onclick="pagecoupons('+i+','+num+')">'+i+' </a>'; }
				 */
				if (start > 2) {
		...
		success : function(data) {
			if (data != "") {
				if (data.pageBean != "") {
					...
					$.each(data.pageBean, function(idx, item) {
						...
						if (item.codeStatus == '0') {
							...
							str += '</td><td><span class="label label-danger">已作废</span>';
						}
				...
				$.each(listg, function(idx, item) {
					...
					strg += '<td>' + item.codeNo;
					strg += '</td><td><span class="label label-default">未使用</span>';
					if (item.codeGetTime == null) {
						strg += '</td><td>';
					} else {
				...
				$.each(listy, function(idx, item) {
					...
					stry += '<td>' + item.codeNo;
					stry += '</td><td><span class="label label-success">使用成功</span>';
					if (item.codeGetTime == null) {
						stry += '</td><td>';
					} else {
