...
function toStep2() {
    ...
    // 验证商品名称是否已经存在
    $.ajax({
        url:"checkTypeName.htm?CSRFToken="+$("#CSRFToken").val()+"&typeName="+$("#typeNamet").val(),
        async:false,
...
function toStep3() {
   ...
   // checkInput("saveTypeForm","expandnicknames");
    checkInput("saveTypeForm","expandparamsort", /^\+?(0|[1-9][0-9]*)$/,"必须为非负整数");
...
function toUpdateStep2() {
    ...
    // 验证商品名称是否已经存在
    $.ajax({
        url:"checkTypeName.htm?CSRFToken="+$("#CSRFToken").val()+"&typeName="+$("#typeName").val()+"&id="+$("#typeId").val(),
        async:false,
        success:function (data) {
            isCanNotNext = data;
        }
...
function toUpdateStep3() {
    ...
    checkInput("updateTypeForm","expandparamsort",/^\+?(0|[1-9][0-9]*)$/,"必须为非负整数");
