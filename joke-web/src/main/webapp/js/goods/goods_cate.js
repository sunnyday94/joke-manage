...
function showAddCate(level) {
    ...
    $('#cateAdd').modal("show");
    $('input[name=catName]').removeClass("error");
    $('input[name=catName]').next().hide();
    $('input[name=catSort]').removeClass("error");
    $('input[name=catSort]').next().hide();
...
function submitSaveGoodsCateForm() {
    ...
    if($("#add_cate_rate").val()<0.01 || $("#add_cate_rate").val()>100){
        $("#rateContainer").append("<label for='add_cate_rate' generated='true' class='error'>0.01~100之间！</label>");
        $("#add_cate_rate").addClass("error");
        return;
    }
...
function submitUpdateGoodsCateForm() {
    ...
    if($("#update_cat_rate").val()<0.01 || $("#update_cat_rate").val()>100){
        $("#update_cat_rate").parent("div").append("<label for='update_cat_rate' generated='true' class='error'>0.01~100之间！</label>");
        $("#update_cat_rate").addClass("error");
        return;
    }
    var level = $("#cur_level").val();
    var parentId = $("#parentId"+(parseInt(level)-1)).val();
