...
function loadFirstCate(parentId, cateName) {
	var url = 'querySonCateByCatIdAndName.htm?CSRFToken='
			+ $("#CSRFToken").val() + '&catId=' + parentId;
	...
	$.ajax({
		...
		success : function(data) {
			...
			for (var i = 0; i < data.length; i++) {
				var cate = data[i];
				html += '<div class="cate_item" id="cate_item' + cate.catId
						+ '" onclick="loadSecondCate(this,' + cate.catId
						+ ',null,\'' + cate.catName + '\')">' + '<h4>'
						+ cate.catName + '</h4>' + '</div>';
			}
			...
			if ($("#cate_list1").find("div.cate_item").length > 0) {
				...
				$("#productName1").text(
						$("#cate_list1").find("div.active h4").html());
	...
	$.ajax({
		...
		success : function(data) {
			...
			for (var i = 0; i < data.length; i++) {
				var cate = data[i];
				html += '<div class="cate_item" id="cate_item' + cate.catId
						+ '" onclick="loadThirdCate(this,' + cate.catId
						+ ',null,\'' + cate.catName + '\')">' + '<h4>'
						+ cate.catName + '</h4>' + '</div>';
			}
			...
			if ($("#cate_list2").find("div.cate_item").length > 0) {
				...
				$("#productName2").text(
						$("#cate_list2").find("div.active h4").html());
			} else {
				$("#cate_list3").html("");
				$("#parentId2").val('');
				$("#parentId3").val('');
	...
	$.ajax({
		...
		success : function(data) {
			...
			for (var i = 0; i < data.length; i++) {
				var cate = data[i];
				html += '<div class="cate_item" id="cate_item' + cate.catId
						+ '" onclick="clickThirdCate(this,' + cate.catId
						+ ',\'' + cate.catName + '\')">' + '<h4>'
						+ cate.catName + '['
						+ (cate.goodsCount == null ? 0 : cate.goodsCount)
						+ '件商品]</h4>' + '</div>';
			}
			...
			if ($("#cate_list3").find("div.cate_item").length > 0) {
				...
				$("#productName3").text(
						$("#cate_list3").find("div.active h4").html());
			} else {
				$("#cate_list3").html("");
				$("#parentId3").val('');
			}
...
function loadTypeParam() {
	...
	$
			.get(
					"queryTypeVoByCatId.htm?catId=" + catId + "&CSRFToken="
							+ $("#token_val").val(),
					function(data) {
						...
						/*
						 * <tr> <td>图案文化</td> <td> <select
						 * class="form-control"> <option>电影</option> <option>创意</option>
						 * <option>青春</option> </select> </td> </tr>
						 */
						...
						if (params.length > 0) {
							for (var i = 0; i < params.length; i++) {
								...
								paramHtml = paramHtml
										...
										+ "</td><td class='"
										+ params[i].paramName
										+ "'><input type='hidden' class='type_param' name='paramId' value='"
										+ params[i].paramId
										+ "'/><input type='text' value='' class='form-control type_param_val' name='paramValue'/></td>";
						...
						$("#parameter tbody").append(paramHtml);
						/* 详细参数END */
						...
						if (specs.length > 0) {
							...
							for (var i = 0; i < specs.length; i++) {
								...
								specHtml += ' <label> <input type="checkbox" class="type_spec"  value="'
										+ specs[i].goodsSpec.specId
										+ '">'
										+ specs[i].goodsSpec.specName
										+ '</label></div>';
								specHtml += '<div class="spec_set_details" style="display:none"  id="emp'
										+ (i + 1) + '" name="no">';
								...
								for (var j = 0; j < specs[i].goodsSpec.specDetails.length; j++) {
									...
									specHtml += '<td><input type="checkbox" class="check_spec openSpecValue_'
											+ specs[i].goodsSpec.specId
											+ '" spec_id="'
											+ specs[i].goodsSpec.specId
											+ '" spec_value_id="'
											+ specs[i].goodsSpec.specDetails[j].specDetailId
											+ '" value="'
											+ specs[i].goodsSpec.specDetails[j].specDetailName
											+ '"  onchange="chImg(this);"></td>';
									specHtml += '<td> <input class="form-control w100" type="text" value="'
											+ specs[i].goodsSpec.specDetails[j].specDetailName
											+ '" onchange="changeValueId(this)"></td>';

									specHtml += '<td>';
									specHtml += ' <div class="spec_value_img" style="display:none;">';
									specHtml += '<a href="javascript:;" class="add_img" onclick="">上传图片</a>';
									specHtml += '<form action="uploadImgSingle.htm" method="post" target="hidden_frame" enctype="multipart/form-data" class="specValue_'
											+ specs[i].goodsSpec.specDetails[j].specDetailId
											+ ' spec_img_form_'
											+ specs[i].goodsSpec.specId + '">';
									specHtml += '<input type="hidden" name="specValId" value="'
											+ specs[i].goodsSpec.specDetails[j].specDetailId
											+ '">';
									specHtml += '<input type="hidden" class="up_spec_img_src up_spec_img_src_'
											+ specs[i].goodsSpec.specDetails[j].specDetailId
											+ '" value="">';
									specHtml += '<input style="position:relative;z-index:99;display:block;width:36px;height:36px;opacity:0;filter:Alpha(opacity=0);overflow:hidden;" type="file" name="specImg" spec_val_id="'
											+ specs[i].goodsSpec.specDetails[j].specDetailId
						...
						// bindEvent();
						// tx();
						$('.spec_set input').change(
								function() {
									if ($(this).is(':checked')) {
										$(this).parent().parent().next().attr(
												'name', 'ok');
...
function loadProduct() {
	...
	if (ch_spec.length == 1) {
		...
		if (spec_vals.length > 0) {
			for (var i = 0; i < spec_vals.length; i++) {
				if ($(spec_vals[i]).prop("checked")) {
					...
					_tabs += '><a href="#tab' + (i + 1)
							+ '" aria-controls="tab' + (i + 1)
							+ '" role="tab" data-toggle="tab">'
							+ $(spec_vals[i]).val() + '</a></li> ';
					var hidearea = "<input type='hidden' class='product_spec' value="
							+ $(spec_vals[i]).attr("spec_id")
							+ "-"
							+ $(spec_vals[i]).attr("spec_value_id")
							+ "-"
							+ $(spec_vals[i]).val() + ">";
					...
					$("#tab" + (i + 1)).find(".batch_set_stock_ctrl").attr(
							"spe_id", i + 1);
					$("#tab" + (i + 1)).find(".do_batch_set_stock_ctrl").attr(
							"spe_id", i + 1);
					$("#tab" + (i + 1)).find(".cancel_batch_set_stock_ctrl")
							.attr("spe_id", i + 1);
					$("#tab" + (i + 1)).find(".copy_ware_to_other_spec_ctrl")
							.attr("spe_id", i + 1);

					$("#tab" + (i + 1)).find(".batch_set_price_ctrl").attr(
							"spe_id", i + 1);
					$("#tab" + (i + 1)).find(".do_batch_set_price_ctrl").attr(
							"spe_id", i + 1);
					$("#tab" + (i + 1)).find(".cancel_batch_set_price_ctrl")
							.attr("spe_id", i + 1);

					$("#tab" + (i + 1)).find(".batc_set_stock").attr("id",
							"batc_set_stock" + (i + 1));
					$("#tab" + (i + 1)).find(".batch_set_price").attr("id",
							"batc_set_price" + (i + 1));

					$("#tab" + (i + 1)).find(".batch_set_price_ctrl").attr(
							"id", "batch_set_price_ctrl" + (i + 1));
					$("#tab" + (i + 1)).find(".batch_set_stock_ctrl").attr(
							"id", "batch_set_stock_ctrl" + (i + 1));
					$("#tab" + (i + 1)).find(".do_batch_set_stock_ctrl").attr(
							"id", "do_batch_set_stock_ctrl" + (i + 1));
					$("#tab" + (i + 1))
							.find(".cancel_batch_set_stock_ctrl")
							.attr("id", "cancel_batch_set_stock_ctrl" + (i + 1));
					$("#tab" + (i + 1)).find(".do_batch_set_price_ctrl").attr(
							"id", "do_batch_set_price_ctrl" + (i + 1));
					$("#tab" + (i + 1))
							.find(".cancel_batch_set_price_ctrl")
							.attr("id", "cancel_batch_set_price_ctrl" + (i + 1));

					$('.productsImg').popover({
						content : '建议350*350px以上',
						trigger : 'hover'
					});

					/*
					 * ct.find(".nameWp input").attr("name","_rad"+ct.index());
					 * ct.find(".customerDiscount
					 * input").attr("name","_discount"+ct.index());
					 */
				}
			}
		}
		...
		if (empArray2.length == 0) {
			for (var x = 0; x < empArray3.length; x++) {
				...
				for (var i = 0; i < titles.length; i++) {
					if (i == 0) {
						...
						hidearea += "<input type='hidden' class='product_spec' value="
								+ titles[i - 2]
								+ "-"
								+ titles[i - 1].split("_")[0]
								+ "-"
								+ titles[i - 1].split("_")[1] + ">";
					}
				}
				var _tabs = '<li role="presentation"';
				_tabs += '><a href="#tab' + (x + 1) + '" aria-controls="tab'
						+ (x + 1) + '" role="tab" data-toggle="tab">' + title
						+ '</a></li> ';
				...
				$("#tab" + (x + 1)).find(".copy_ware_to_other_spec_ctrl").attr(
						"spe_id", x + 1);
				$("#tab" + (x + 1)).find(".batch_set_stock_ctrl").attr(
						"spe_id", x + 1);
				$("#tab" + (x + 1)).find(".do_batch_set_stock_ctrl").attr(
						"spe_id", x + 1);
				$("#tab" + (x + 1)).find(".cancel_batch_set_stock_ctrl").attr(
						"spe_id", x + 1);

				$("#tab" + (x + 1)).find(".batch_set_price_ctrl").attr(
						"spe_id", x + 1);
				$("#tab" + (x + 1)).find(".do_batch_set_price_ctrl").attr(
						"spe_id", x + 1);
				$("#tab" + (x + 1)).find(".cancel_batch_set_price_ctrl").attr(
						"spe_id", x + 1);

				$("#tab" + (x + 1)).find(".batc_set_stock").attr("id",
						"batc_set_stock" + (x + 1));
				$("#tab" + (x + 1)).find(".batch_set_price").attr("id",
						"batc_set_price" + (x + 1));

				$("#tab" + (x + 1)).find(".batch_set_price_ctrl").attr("id",
						"batch_set_price_ctrl" + (x + 1));
				$("#tab" + (x + 1)).find(".batch_set_stock_ctrl").attr("id",
						"batch_set_stock_ctrl" + (x + 1));
				$("#tab" + (x + 1)).find(".do_batch_set_stock_ctrl").attr("id",
						"do_batch_set_stock_ctrl" + (x + 1));
				$("#tab" + (x + 1)).find(".cancel_batch_set_stock_ctrl").attr(
						"id", "cancel_batch_set_stock_ctrl" + (x + 1));
				$("#tab" + (x + 1)).find(".do_batch_set_price_ctrl").attr("id",
						"do_batch_set_price_ctrl" + (x + 1));
				$("#tab" + (x + 1)).find(".cancel_batch_set_price_ctrl").attr(
						"id", "cancel_batch_set_price_ctrl" + (x + 1));
				 ...
				 * var ct = $(".demo_box").clone(); ct.removeClass("demo_box");
				 * $(".dinfo_tabs").append(_tabs); ct.append(hidearea);
				 * $(".dinfo_wp").append(ct); ct.find(".nameWp
				 * input").attr("name","_rad"+ct.index());
				 * ct.find(".customerDiscount
				 * input").attr("name","_discount"+ct.index());
				 */
			}
	...
	// $(".dinfo_box:eq(0)").append('<div class="tc"><a class="j_btn cp_all"
	// href="javascript:;" onclick="cp_all(this);">复制到全部</a></div>');
	/* 绑定事件 */
	// bindEvent();
	addEvent();
	defaultActive();
...
function saveProduct() {
	...
	if($($(".des_input")[0]).val() != undefined && $($(".des_input")[0]).val() != 'undefined'){
		$(".save_goods_info").append("<input type='hidden' name='goodsSubtitle' value='" + $($(".des_input")[0]).val() + "' />");
	}
	
	$(".save_goods_info").append("<input type='hidden' name='thirdId' value=" + $("#thirdId").val() + " >");
	...
	if (null != specVal && specVal.length > 0) {
		for (var i = 0; i < specVal.length; i++) {
			...
			/* 保存规格别名 */
			$(".save_goods_info").append("<input type='hidden' name='specsValueRemark' value=" + $($(specVal[i])).val().replace(/\s+/g,'&nbsp;') + " >");
		}
	}
...
function call_save_goods(data) {
	...
	var thirdId = result.thirdId;
	
	if (result.goodsId > 0) {
		/* 保存完基本信息之后保存详细介绍,完成之后保存货品信息 */
		$(".new_third_id").val(thirdId);
		$(".new_goods_id").val(goodsId);
...
function savePro(data) {
	...
	if (null != pro_box && pro_box.length > 0) {
		for (var j = 0; j < pro_box.length; j++) {
			...
			if (_pro.find(".product_spec").length > 0) {
				...
				var param = "";
				param += "goodsId=" + goodsId + "&thirdId=" + thirdId;
				...
				if(_pro.find(".des_input").val() != undefined && _pro.find(".des_input").val() != 'undefined'){
				 ...
				 * param+="&showMobile=1"; }else{ param+="&showMobile=0"; }
				 */
	...
	saveGoodsOk("newUploadGoods.htm", "findAllGoods.htm?menuId=3&menuParentId=7&myselfId=18", "添加商品成功！");
...
function specImgSucc(msg) {
	if (msg == "111") {
		...
		if ($(".up_spec_img_" + id)[0]) {
			...
			$(".specValue_" + id).parent().before("<div class='spec_value_img up_spec_img_" + id
							+ "' style='background:#e2e2e2 url(" + url + ") no-repeat center center'></div> ");
		}
...
function callSaveChoooseImage(url, width, height) {
	...
	$(".dinfo_wp")
			...
			.find(".choose_imgs")
			.append(
					'<span class="inline"><a class="del_ts" href="javascript:;" onclick="del_ts(this)"><i class="glyphicon glyphicon-remove-sign"></i></a><input type="hidden" class="choose_img" value='
							+ url
							+ '><a href="javascript:;"><img alt="" class="choose_pro_img" src='
							+ url + ' width="100" height="100" /></a></span>');
...
function callSaveChoooseImageBath(urls, width, height, currentIndex, allSize) {
	...
	if (currentIndex == allSize - 1) {
		if (errorIds.length == 0) {
			...
			showConfirmAlert('要将这些图片应用到其他货品吗?', 'copyImageToOtherSpec(\''
					+ urls + '\')');
		} else {
			...
			if (errorIds.length == urls.length) {
				...
				showConfirmAlert('第' + errorIds
						+ '张图片尺寸过小！建议尺寸350*350以上,要将符合的图片应用到其他货品吗?',
						'copyImageToOtherSpec(\'' + rightUrls + '\')');
			}

		}

	} else {
		imgLoadbath(urls[currentIndex + 1], urls, callSaveChoooseImageBath,
...
function saveGoodsOk(addUrl, listUrl, tips) {
	...
	confirmDialog += '           </div>'
			...
			+ '               <button type="button" class="btn btn-primary" onclick="jumpUrl(\''
			+ addUrl
			+ '\');">继续添加</button>'
			+ '               <button type="button" class="btn btn-default" onclick="jumpUrl(\''
			+ listUrl + '\');">商品列表</button>' + '           </div>'
			+ '       </div>' + '   </div>' + '</div>';
...
function queryGoodsByParam(pageNo) {
	...
	$
			.get(
					"queryByParamAjax.htm?goodsCateId=" + $('#parentId3').val()
							+ "&pageNo=" + pageNo + "&goodsNo=" + goodsNo
							+ "&goodsName=" + goodsName + "&goodsBrandId="
							+ goodsBrandId + "&showFlag=1&CSRFToken="
							+ $("#CSRFToken").val(),
					function(data) {
						var aboutGoodsHtml = "";
						var list = data.list;
						if (list.length > 0) {
							for (var i = 0; i < list.length; i++) {
								aboutGoodsHtml = aboutGoodsHtml
										+ "<tr><td><input type='checkbox' goods_name='"
										+ list[i].goodsName
										+ "' goods_no='"
										+ list[i].goodsNo
										+ "' goods_img='"
										+ list[i].goodsImg
										+ "' class='form-control' name='aboutGoodsIdSelect' value='"
										+ list[i].goodsId
										+ "'/></td>"
										...
										+ " /></td><td title='"
										+ list[i].goodsNo + "'>"
										+ list[i].goodsNo + "</td><td>"
										+ list[i].goodsName + "</td>";
								aboutGoodsHtml = aboutGoodsHtml + "<td>"
										+ list[i].goodsType.typeName
										+ "</td><td>"
										+ list[i].goodsBrand.brandName
										+ "</td></tr>";
							}
						} else {
							aboutGoodsHtml = aboutGoodsHtml
									+ "<tr><td colspan='6'>选择的分类下暂时还没有商品</td></tr>";
						}
						...
						if (data.pageNo == 1) {
							...
							foot += "<a href='javascript:void(0)' onclick='queryGoodsByParam("
									+ data.prePageNo + ")'>上一页</a>";
						}
						var i = 1;
						for (var l = data.startNo; l <= data.endNo; l++) {
							if ((i - 1 + data.startNo) == data.pageNo) {
								...
								foot = foot + "<a onclick='queryGoodsByParam("
										+ (i - 1 + data.startNo)
...
function saveRelGoods() {
	...
	$("input[name=aboutGoodsIdSelect]:checkbox")
			.each(
					function() {
						if ($(this).is(':checked') == true) {
							selecteOne = true;
							if ($("#selectedGoods" + $(this).val()) == undefined
									|| $("#selectedGoods" + $(this).val()).length <= 0) {
								html += '<tr id="rel_goods_tr'
										+ $(this).val()
										+ '">'
										+ '   <td><img src="'
										+ $(this).attr("goods_img")
										+ '" width="50px"></td>'
										+ '   <td><input type="hidden" name="aboutGoodsId" value="'
										+ $(this).val()
										+ '"/><input type="hidden" id="selectedGoods'
										+ $(this).val()
										+ '" value="'
										+ $(this).val()
										+ '"/>'
										...
										+ '   </td>'
										+ '   <td><span>'
										+ $(this).attr("goods_name")
										+ '</span></td>'
										+ '   <td><span><a href="javascript:;" onclick="delRelGood($(\'#rel_goods_tr'
										+ $(this).val()
										+ '\'))">删除</a></span></td>' + '</tr>'
							}
						}
...
function copyWareToOtherSpec(obj) {
	...
	if (null != pro_box && pro_box.length > 0) {
		for (var j = 0; j < pro_box.length; j++) {
			...
			if (null != wares && wares.length > 0) {
				for (var i = 0; i < wares.length; i++) {
					$(_pro.find(".ware_stock")[i]).val(
							from_pro.find(".ware_stock")[i].value);
					$(_pro.find(".ware_price")[i]).val(
							from_pro.find(".ware_price")[i].value);
				}
			}
		}
	}
...
function checkProNo(obj) {
	...
	if ($(obj).val() != undefined && $(obj).val().trim().length > 0) {
		var param = "checkProductNo.htm?productNo=" + $(obj).val() + "&time="
				+ Math.random();
		$.ajax({
			type : 'post',
			url : param,
...
function generateOtherProNo(proNo) {
	var proNoInput = $(".dinfo_wp").find(".tab-pane").not(".active").find(
			".p_code");
