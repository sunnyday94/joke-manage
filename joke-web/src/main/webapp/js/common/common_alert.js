...
function doAjaxDeleteOne(deleteUrl) {
	if (deleteUrl != null) {
		if($('input[name="searchText"]').val() != '' && $('input[name="searchText"]').val() != undefined){
 ...
 * 
 */
function showSendCouponAllConfirmAlert(sendCouponAllUrl,tips) {
    $("#modalDialog").remove();
    var confirmDialog =
    '<div class="modal fade" id="modalDialog" tabindex="-1" role="dialog">'+
    '    <div class="modal-dialog">'+
    ...
    if(tips!='' && tips!=undefined){
        ...
        confirmDialog +='确认要给所有用户发送优惠券吗？';
    }
    confirmDialog += '           </div>'+
    '           <div class="modal-footer">'+
    '               <button type="button" class="btn btn-primary" onclick="doAjaxSendCouponAll(\''+sendCouponAllUrl+'\');">确定</button>'+
    '               <button type="button" class="btn btn-default" data-dismiss="modal" onclick="$(\'#modalDialog\').modal(\'hide\');">取消</button>'+
    '           </div>'+
    '       </div>'+
    '   </div>'+
