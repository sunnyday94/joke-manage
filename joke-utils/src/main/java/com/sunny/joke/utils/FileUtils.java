...
public class FileUtils extends org.apache.commons.io.FileUtils {
	...
	public static String uuidFileName(String fileName) {
		return UUID.randomUUID().toString().replaceAll("-", "") + "." + getFileType(fileName);
	}
	 ...
	 * @description:
	 *               <p class="detail">
	 *               获取一个目录下所有的文件（包括子目录的文件）
	 *               </p>
	 * @author: <a href="mailto:wangdanhua@chunmi.com ">王丹华</a>
	 * @date: Feb 17, 2017-11:21:56 AM
	 * @param @param
	 *            path
	 * @return void
	 */
	public static List<String> listFiles(String path) {
		...
		if (file.isFile()) {
			throw new RuntimeException(path + "必须为一个目录");
		}
		if (file.exists()) {
			...
			while (!list.isEmpty()) {
				...
				for (File file2 : files) {
					if (file2.isDirectory()) {
						...
						docs.add(file2.getAbsolutePath());
					}
				}
			}
		} else {
